...

{
  ...
  "mainSlides": [
    {
      "title": "ページタイトル（簡潔に）",
      "imageDesc": "このスライドにふさわしい画像の詳細な説明（AI画像生成プロンプト用。なるべく詳細に記述する。画像内に表示する日本語テキストがあれば、そのテキストを日本語のまま明記すること（例: Display the text 「設定画面を開く」 in Japanese）。手順・ハウツー系ならスクショ風UI画像を指定、それ以外はイメージ画像）",
...
- imageDescはなるべく詳細に、具体的なビジュアルを描写する（AI画像生成で使うため）。以下のルールを厳守:
  【日本語テキストの扱い（最重要）】画像内に表示すべき日本語テキスト（ラベル、ボタン名、見出し、キーワード等）がある場合は、プロンプト内にそのテキストを日本語のまま明記すること。例: Display the Japanese text 「AIで時短」 / Show label 「設定」 / Button text reads 「開始」。日本語テキストを英訳せず日本語のまま書くことで文字化けを防ぐ。
  ...
  【手順・ハウツーでないスライドの場合】通常のイメージ画像（コンセプトイラスト、アイコン的表現など）を指定する。"Screenshot-style"は使わない。図解やインフォグラフィックの場合は、表示するキーワードやラベルを日本語で明記する。
